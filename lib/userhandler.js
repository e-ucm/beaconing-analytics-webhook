...
module.exports = (function () {
    ...
    userHandler.transformParticipants = function(participants, callback){
        var transfomed = {
            teachers: [],
            students: []
        };
        ...
        total = participants.teachers.length + participants.students.length;

        if(participants.teachers){
            for(var i = 0; participants.teachers.length; i++){
                userHandler.getUser(participants.teachers[i], config, participantCompleted('teacher'));
            }
        }

        if(participants.students){
            for(var i = 0; participants.students.length; i++){
