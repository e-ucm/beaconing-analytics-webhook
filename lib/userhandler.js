...
module.exports = (function () {
    ...
    userHandler.created = function(user, config, callback){
        var a2user = {
            username: user.username,
            email: user.id + user.username + '@beaconing.eu',
            name: {first: user.fullname},
            roles: user.roles,
            prefix: 'gleaner',
            externalId: [{ domain: 'beaconing', id: user.id}]
        }
    ...
    userHandler.auth = function(config, callback){
        request({
            ...
            headers.Authorization = body.user.token;
