...
module.exports = (function () {
    ...
    glpHandler.assigned = function(glp, callback){
        ...
        var Completed = function(){
        	...
        	if(completed >= total){
        		console.log("YA ESTA PODEMOS HACER ELASTIC AQUI");
        		callback(null, {message: 'todo correcto' + });
        	}
        }
		...
		glpHandler.createActivity(content._id + '_' + content.name, '5a96cf871c4f4b0076dbe0f8', '5a96cfe21c4f4b0076dbe0fa', 'teacher', null, content, config, 
			function(error, result){
				...
				/*for (var id in nodes.length) {
					glpHandler.createActivity(content._id + '_' + id + '_' + nodes[id].name, '5a96cf871c4f4b0076dbe0f8', '5a96cfe21c4f4b0076dbe0fa', 'teacher', result._id, nodes[id], config, Completed);
				}*/
			});
    };

    glpHandler.createActivity = function(name, gameId, classId, teacher, rootId, original_node, config, callback){
    	request({
