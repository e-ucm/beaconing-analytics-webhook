...
module.exports = (function () {
    ...
    glpHandler.assigned = function(body, classId, teacher, config, esClient, queue, callback){
                        ...
                        if(error){
                            ...
                            if(error){
                                ...
                                var queue_event = {
                                    ...
                                    body: {
                                        activityId: rootActivity._id, 
                                        dashboard: JSON.parse(dashboardTemplate(rootActivity._id)),
                                        config: config
                                    }
                                };
                                queue.Send(JSON.stringify(queue_event), function(error, result){
                                    if(error){
                                        console.log("error on update root dashboard enqueue");
                                        return callback(error);
                                    }else{
                                        for (var id in nodes) {
                                                            ...
                                                            if(error){
                                                                ...
                                                                var queue_event = {
                                                                    ...
                                                                    body: {
                                                                        ...
                                                                        dashboard: JSON.parse(dashboardTemplate(result._id)),
                                                                        config: config
                                                                    }
                                                                };
                                                                queue.Send(JSON.stringify(queue_event), function(error, result){
                                                                    if(error){
                                                                        console.log("error on update dashboard enqueue");
                                                                        return callback(error);
                                                                    }else{
                                                                        Completed();
                                                                    }
    ...
    glpHandler.createActivity = function(name, gameId, versionId, classId, teacher, rootId, original_node, config, callback){
    	...
    	request({
            ...
            getAnalytics(original_node).dashboard = 'https://rage.e-ucm.es/api/proxy/kibana/app/kibana#/dashboard/dashboard_' +
