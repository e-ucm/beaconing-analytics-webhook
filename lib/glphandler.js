...
module.exports = (function () {
    ...
    /**
     * Updates a Dashboard for an activity, replacing the colocation of the visualizations and configuration
     * @param  {string}   activityId ID of the activity for the dashboard to be replaced
     * @param  {object}   dashboard  The Kibana Dashboard object
     * @param  {object}   config     App config for urls and auth
     * @param  {Function} callback   Callback function to call when everything is completed
     * @return {object}              Null or error if so.
     */
                ...
                if(error){
                    ...
                    glpHandler.startActivity(activity._id, teacher, config, function(error, result){
                        if(error){
                            ...
                            glpHandler.updateDashboard(activity._id, JSON.parse(dashboardTemplate(activity._id)), config, function(error, result){
                                if(error){
    ...
    glpHandler.insertStructure = function(rootId, activityId, body, esClient, callback){
        ...
        for(var l in body.contributes.learningObjectives){
            if(body.contributes.learningObjectives[l].name !== ''){
                var newname = body.contributes.learningObjectives[l].name.replace('.', '');
                contributes.learningObjectives[newname] = body.contributes.learningObjectives[l].percentage;
            }
        }

        for(var c in body.contributes.competences){
            if(body.contributes.competences[c].name !== ''){
                var newname = body.contributes.competences[c].name.replace('.', '');
                contributes.competencies[newname] = body.contributes.competences[c].percentage;
            }
        }
