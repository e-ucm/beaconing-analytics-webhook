...
module.exports = (function () {
    ...
    glpHandler.assigned = function(glp, config, esClient, callback){
		...
		glpHandler.createActivity(content.id + '_(' + content.name + ')', '5a9d8b51d4d13a0077ddb99b', '5a9d8b51d4d13a0077ddb99c', '5a9d8bd0d4d13a0077ddb99d', 'teacher', null, content, config, 
			function(error, result){
				...
				for (var id in nodes) {
					...
					glpHandler.createActivity(content.id + '_' + id + '_(' + nodes[id].name + ')', '5a9d8b51d4d13a0077ddb99b', '5a9d8b51d4d13a0077ddb99c', '5a9d8bd0d4d13a0077ddb99d', 'teacher', result._id, nodes[id], config, 
						function(error, result){
							if(error){
								console.log("error creating root");
								return callback(error);
							}else{
                                glpHandler.startActivity(result._id, teacher, function(errror, result){
                                    if(error){
    ...
    glpHandler.startActivity = function(activityId, teacher, callback){
        request({
            uri: config.backendUrl + 'api/activities/' + activityId + '/start',
            method: 'POST',
            body: {},
            json: true,
            headers: {
    ...
    glpHandler.insertStructure = function(rootId, activityId, body, esClient, callback){
        ...
        for(var l in body.contributes.learningObjectives){
           contributes.learningObjectives[body.contributes.learningObjectives[c].name] = body.contributes.learningObjectives[c].percentage;
        }
