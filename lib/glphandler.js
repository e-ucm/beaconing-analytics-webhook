...
module.exports = (function () {
    ...
    glpHandler.createTreeInElastic = function(glp, total, esClient, callback){
		...
		for(var i in content.missions){
			var mission_childs = [];
			glp_childs.push(content.missions[i].analytics.activityId);

			var current_mission = content.missions[i];

			for(var j in content.missions[i].quests){
				mission_childs.push(content.missions[i].quests[j].analytics.activityId);
				...
				for(var k in content.missions[i].quests[j].graph.quest.challenges){
					...
					for(var l in content.missions[i].quests[j].graph.quest.challenges[k].activities){
						var current_activity = content.missions[i].quests[j].graph.quest.challenges[k].activities[l];
						quest_childs.push(content.missions[i].quests[j].graph.quest.challenges[k].activities[l].analytics.activityId);

						glpHandler.insertStructure(rootId, content.missions[i].quests[j].analytics.activityId, 
						{
							limits: current_activity.analytics.json.limits,
							contributes: current_activity.json.contributions,
				...
				glpHandler.insertStructure(rootId, content.missions[i].quests[j].analytics.activityId, 
				{
					...
					contributes: current_quest.analytics.activityId.json.contributions,
			...
			glpHandler.insertStructure(rootId, current_mission.analytics.activityId, 
			{
				...
				contributes: current_mission.analytics.json.contributions,
		...
		glpHandler.insertStructure(rootId, glp.analytics.activityId, 
		{
			...
			contributes: glp.analytics.json.contributions,
			children: glp_childs
		}, cb);
    }

    glpHandler.insertStructure = function(rootId, activityId, body, callback){
